...
{{--  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.1.3/css/bootstrap.min.css" />  --}}
   ...
   <div class="container-fluid tm-container-content tm-mt-60" style="padding: 30px;margin-top: -3%;">
            ...
            <div class="row mb-4">
                <h1 class="text-center" style="font-size: 1.5rem;margin-top: 50px;font-weight: 400;">{{__('auth.customer_registration_form_create_account')}}</h1>
        <div class="row">
                    ...
                    @csrf
                      ...
                      <div class="form-group col-md-4 " style="padding: 0;">
                        <select style="border: solid 1px lightgrey;border-radius: 4px;color: grey !important;height: 6%;" data-message="Bitte w채hlen Sie die Option aus der Liste" name="title" id="data-message" class=" @error('title') is-invalid @enderror">
                                <option value="">-</option>
                                <option {{ old('title') == 'mr'  ? 'selected' : '' }} value="mr">{{__('auth.customer_registration_form_input_title_mr')}}</option>
                                <option {{ old('title') == 'mrs' ? 'selected' : '' }}  value="mrs">{{__('auth.customer_registration_form_input_title_mrs')}}</option>
                            </select>
                            @error('title')
                      ...
                      <div class="form-group">
                        <input  type="text" value="{{ old('name') }}"
                            ...
                            class="form-control @error('name') is-invalid @enderror" data-message="Bitte geben Sie in das Feld etwas ein" placeholder="{{__('auth.customer_registration_form_input_name')}}">
                    @error('name')
                        <span class="invalid-feedback error" role="alert">
                    ...
                    @enderror
                      ...
                      <div class="form-group">
                        <input  type="text" value="{{ old('last_name') }}"
                                   name="last_name"
                                   class="form-control @error('last_name') is-invalid @enderror" data-message="Bitte geben Sie in das Feld etwas ein" placeholder="{{__('auth.customer_registration_form_input_last_name')}}">
                            @error('last_name')
                                <span class="invalid-feedback error" role="alert">
                                    <strong>{{ $message }}</strong>
                      ...
                      <div class="form-group">
                         <input  type="email" value="{{ old('email_confirmation') }}"
                                   name="email_confirmation"
                                    ...
                                    oninput="this.setCustomValidity('')"--}}
                                   class="form-control @error('email') is-invalid @enderror" data-message="Bitte geben Sie in das Feld etwas ein" placeholder="{{__('auth.customer_registration_form_input_email_confirm')}}">
                       ...
                       <div class="form-group">
                        ...
                        <button type="submit"  class="btn btn-secondary btn-sm" style="background-color:#048BA8;">Speichern und weiter</button>
                      </div>
</form>
        ...
        or <span><a class="" href="{{ url('/') }}" style="color: #048ba8;font-size: 14px;">{{__('auth.customer_registration_form_return_to_store')}}</a></span>
                   ...
                   <p class="text-gray text-sm text-center">Haben Sie schon ein Kundenkonto? <a href="{{route('customer.login_form')}}"><u>Hier einloggen</u></a></p>
                </div>
             </div>
          </div>
